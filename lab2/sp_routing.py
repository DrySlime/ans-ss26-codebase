...
class SPRouter(app_manager.RyuApp):
    ...
    def _get_dpid_from_ip(self, ip_str):
        try:
            ...
            # Extrahieren der IP-Komponenten: 10.A.B.C
            a = int(octets[1])
            b = int(octets[2])
            c = int(octets[3])
            
            # 1. Fall: Core-Switch (Format: 10.k.j.i)
            if a == self.k:
                j = b
                i = c
                core_idx = (i - 1) * n + (j - 1)
            ...
            # 2. Fall: Pod-Switches (Format: 10.pod.switch_id.1)
            # In einem Fat-Tree hat ein Switch im Pod typischerweise das Suffix .1
            elif c == 1:
                pod = a
                switch_id = b
                
                if switch_id < n:
                    # Edge-Switch (switch_id = edge_idx)
                    agg_limit = core_limit + (self.k * n)
                    edge_idx = switch_id
                    return agg_limit + (pod * n) + edge_idx + 1
                else:
                    # Aggregation-Switch (switch_id = agg_idx + n)
                    agg_idx = switch_id - n
                    return core_limit + (pod * n) + agg_idx + 1
