...
def run_test(coll, rank, world, size, op, loss_rate):
    ...
    if DEBUG:
        print(f"\n--- [Rank {rank}] Hierarchical Test ({op.upper()}): Size {size}, Loss {loss_rate*100:.0f}% ---", flush=True)
...
if __name__ == "__main__":
    ...
    sys.stdout.flush()
    print(f"=== Starting Hierarchical AllReduce Test Suite for Rank {args.rank} (World Size {args.world}) ===")
    ...
    print("\n==============================================")
    if success:
        print(f"=== [Rank {args.rank}] ALL HIERARCHICAL TESTS PASSED! ===")
    else:
        print(f"=== [Rank {args.rank}] SOME HIERARCHICAL TESTS FAILED! ===")
    print("==============================================")
