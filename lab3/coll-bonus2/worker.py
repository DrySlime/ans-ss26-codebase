...
class MyCollectives(Collectives):
    def __init__(self, rank, world):
        ...
        # Bind to port 9999 to receive broadcast responses from the local ToR switch
        self.soc.bind(('', 9999))
    ...
    def AllReduce(self, input: list[int], output: list[int], op : str = "sum"):
        ...
        op_clean = op.lower()
        op_map = {"sum": 0, "avg": 0, "min": 1, "max": 2}
        if op_clean not in op_map:
            raise ValueError(f"Unsupported operator: {op}")
        op_code = op_map[op_clean]
        ...
        self.log(f"Starting Hierarchical AllReduce ({op}) with {num_chunks} chunks (vector size {num_elements})")
        ...
        def send_chunk(c_idx):
            ...
            # Format: chunk_id (I), rank (H), world (H), chunk_len (H), op (H), val0..val5 (iiiiii)
            data = struct.pack(">IHHHHiiiiii", global_chunk_id, self.rank, self.world, chunk_len, op_code, *vals)
        ...
        while base_chunk_idx < num_chunks:
            ...
            while True:
                try:
                    ...
                    unpacked = struct.unpack(">IHHHHiiiiii", res_data)
                    res_chunk_id, res_rank, res_world, res_chunk_len, res_op = unpacked[:5]
                    res_vals = unpacked[5:]
                    received_any = True
                    ...
                    if 0 <= c_idx < num_chunks:
                        if not completed[c_idx]:
                            ...
                            while base_chunk_idx < num_chunks and completed[base_chunk_idx]:
                                ...
                                for i in range(clen):
                                    if op_clean == "avg":
                                        output[pos + i] = res_vals[i] // self.world
                                    else:
            ...
            # 4. If no packets were read in this iteration, yield CPU
            if not received_any:
                time.sleep(0.001)
        ...
        self.log(f"Finished Hierarchical AllReduce ({op}) in {elapsed_total_ms:.1f}ms")
...
if __name__ == "__main__":
    ...
    current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
    print(f"[{current_time}] --- Running Hierarchical AllReduce tests for rank {args.rank} (world {args.world}) ---")
    Test.test_allreduce_all(coll, args.rank, args.world, 66)
